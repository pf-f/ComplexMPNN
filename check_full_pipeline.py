...
def check_preprocessing():
    ...
    if os.path.exists(mpnn_pt_dir):
        ...
        if len(pt_files) >= 10:
            checks.append(("MPNN .pt文件", "✅", f"{len(pt_files)}个文件"))
        else:
            checks.append(("MPNN .pt文件", "⚠️", f"只有{len(pt_files)}个文件（期望>=10）"))
    else:
        checks.append(("MPNN .pt文件", "❌", "目录不存在"))
    ...
    if os.path.exists(interface_masks_dir):
        ...
        if len(mask_files) >= 10:
            checks.append(("界面掩码文件", "✅", f"{len(mask_files)}个文件"))
        else:
            checks.append(("界面掩码文件", "⚠️", f"只有{len(mask_files)}个文件（期望>=10）"))
    else:
        checks.append(("界面掩码文件", "❌", "目录不存在"))
    ...
    if os.path.exists(split_dir):
        for split_file in ['train.txt', 'val.txt', 'test.txt']:
            ...
            if os.path.exists(split_path):
                ...
                checks.append((f"数据集切分 {split_file}", "✅", f"{len(lines)}个样本"))
            else:
                checks.append((f"数据集切分 {split_file}", "❌", "文件不存在"))
    else:
        checks.append(("数据集切分", "❌", "目录不存在"))
...
def check_training():
    ...
    if os.path.exists(checkpoint_dir):
        ...
        if os.path.exists(best_model):
            ...
            checks.append(("最佳模型 checkpoint", "✅", f"{size_mb:.2f} MB"))
            ...
            try:
                ...
                if isinstance(model_state, dict):
                    ...
                    checks.append(("模型参数加载", "✅", f"{param_count:,} 个参数"))
            except Exception as e:
                checks.append(("模型参数加载", "❌", str(e)))
        else:
            checks.append(("最佳模型 checkpoint", "❌", "文件不存在"))
        
        # 检查定期checkpoint
        epoch_files = [f for f in os.listdir(checkpoint_dir) if f.startswith('complexmpnn_epoch_') and f.endswith('.pt')]
        if epoch_files:
            checks.append(("定期 checkpoint", "✅", f"{len(epoch_files)} 个epoch文件"))
        else:
            checks.append(("定期 checkpoint", "⚠️", "未找到epoch文件"))
    else:
        checks.append(("Checkpoint目录", "❌", "目录不存在"))
    ...
    if os.path.exists(eval_dir):
        ...
        for csv_file in csv_files:
            ...
            if os.path.exists(csv_path):
                try:
                    ...
                    checks.append((f"CSV文件 {csv_file}", "✅", f"{len(df)} 行"))
                except Exception as e:
                    checks.append((f"CSV文件 {csv_file}", "❌", str(e)))
            else:
                checks.append((f"CSV文件 {csv_file}", "❌", "文件不存在"))
        ...
        for png_file in png_files:
            ...
            if os.path.exists(png_path):
                ...
                checks.append((f"可视化 {png_file}", "⚠️", "文件不存在"))
    else:
        checks.append(("评估目录", "❌", "目录不存在"))
...
def check_logs():
    ...
    for log_file in log_files:
        if os.path.exists(log_file):
            ...
            checks.append((f"日志 {log_file}", "⚠️", "文件不存在"))
...
def print_summary(prep_ok, train_ok, eval_ok, logs_ok):
    ...
    print(f"预处理数据:   {'✅ 通过' if prep_ok else '❌ 失败'}")
    print(f"训练结果:     {'✅ 通过' if train_ok else '❌ 失败'}")
    print(f"评估结果:     {'✅ 通过' if eval_ok else '❌ 失败'}")
    print(f"日志文件:     {'✅ 检查完成' if logs_ok else '⚠️ 部分缺失'}")
    
    print()
    if prep_ok and train_ok and eval_ok:
        print("🎉 所有检查通过！全流程验证成功！")
        return True
    else:
